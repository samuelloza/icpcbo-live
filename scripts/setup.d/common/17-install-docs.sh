...
CONTEST_DOC_DST="/usr/share/doc/contest"
...
mkdir -p "${CONTEST_DOC_DST}"

if [ -d "${CPPREF_DST}/reference/en" ]; then
    ln -sfn "${CPPREF_DST}/reference/en" "${CONTEST_DOC_DST}/cppreference"
fi

if [ -d /usr/share/doc/python3/html ]; then
    ln -sfn /usr/share/doc/python3/html "${CONTEST_DOC_DST}/python-docs"
fi

if [ -d /usr/share/doc/openjdk-21-doc/api ]; then
    ln -sfn /usr/share/doc/openjdk-21-doc/api "${CONTEST_DOC_DST}/java-docs"
fi
